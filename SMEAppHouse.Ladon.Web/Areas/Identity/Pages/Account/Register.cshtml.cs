...
namespace SMEAppHouse.Ladon.Web.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public RegisterModel(
            IAuthMembershipService userManager,
        ...
        public async Task OnGetAsync(string returnUrl = null)
        {
            ...
            {
                ReturnUrl = returnUrl;
                ExternalLogins = (await _authService.SignInManager.GetExternalAuthenticationSchemesAsync()).ToList();
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    await _authService.UserStore.SetUserNameAsync(user, Input.Email, CancellationToken.None);
                    await _authService.EmailStore.SetEmailAsync(user, Input.Email, CancellationToken.None);
                    var result = await _authService.CreateAsync(user, Input.Password);
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        if (_authService.Options.SignIn.RequireConfirmedAccount)
                        {
        ...
        {
            if (!_authService.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<IdentityUser<Guid>>)_authService.UserStore;
        }
    }
}
